...
func TestEffectiveDSN(t *testing.T) {
	d := validConfig().DB
	got := d.EffectiveDSN()
	for _, want := range []string{"host=127.0.0.1", "port=5432", "dbname=im_account", "sslmode=disable"} {
	...
	d.DSN = "postgres://user:pass@cloud-host:5432/im_account"
...
// 缺 DB 会在第一次请求时报错，缺 JWT_SECRET 会让令牌校验全部失败。
...
//
// 这是可用性与正确性之间的取舍：缓存不可用时登录、注册、资料查询仍可用，
// 只是多设备在线上限退化为不限制。若把它也设为硬依赖，
...
func TestOptionalComponentsOnlyWarn(t *testing.T) {
	...
	if fatal := c.FatalIssues(); len(fatal) != 0 {
		t.Fatalf("缓存缺失不应产生致命问题，得到: %+v", fatal)
	}
...
// TestTokenModeValidation 验证令牌校验模式的取值约束。
//
// 取值写错会让服务在「以为开启了严格校验」的假设下退化为 self 模式，
// 被踢下线的设备仍能继续使用，属于静默的安全降级，必须拒绝启动。
func TestTokenModeValidation(t *testing.T) {
	for _, mode := range []string{TokenModeSelf, TokenModeDB} {
		c := validConfig()
		c.App.TokenMode = mode
		if err := c.Validate(); err != nil {
			t.Errorf("TOKEN_MODE=%q 应当被接受: %v", mode, err)
		}
	}

	for _, mode := range []string{"", "strict", "DB", "none"} {
		c := validConfig()
		c.App.TokenMode = mode
		if err := c.Validate(); err == nil {
			t.Errorf("TOKEN_MODE=%q 应当被拒绝", mode)
		}
	}
}

// TestMaxDevicesBoundary 验证设备数上限的边界取值。
//
// 0 表示「不限制」，是合法配置；负数没有意义，必须拒绝——
// 否则会被当成 0 处理，让「想限制却限制了 0 台」的配置静默变成不限制。
func TestMaxDevicesBoundary(t *testing.T) {
	c := validConfig()
	c.App.MaxDevicesPerUser = 0
	if err := c.Validate(); err != nil {
		t.Fatalf("上限为 0（不限制）应当被接受: %v", err)
	}

	c = validConfig()
	c.App.MaxDevicesPerUser = 1
	if err := c.Validate(); err != nil {
		t.Fatalf("上限为 1 应当被接受: %v", err)
	}
...
// TestDBModeWithCacheWarns 验证 db 模式缺少缓存时给出降级告警。
//
// db 模式每次校验都要回查，缓存不可用会让请求全部落到数据库上。
// 此时仍能工作（只是变慢），因此只告警不阻断启动。
func TestDBModeWithCacheWarns(t *testing.T) {
	c := validConfig()
	c.App.TokenMode = TokenModeDB
	c.Cache.Addrs = nil

	if fatal := c.FatalIssues(); len(fatal) != 0 {
		t.Fatalf("db 模式缺缓存不应产生致命问题，得到: %+v", fatal)
	}

	var found bool
	for _, i := range c.Check() {
		if strings.Contains(i.Message, "TOKEN_MODE=db") {
			found = true
		}
	}
	if !found {
		t.Fatal("db 模式缺缓存应当给出专门的降级提示")
	}
}
