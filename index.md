...
| `4007` | 令牌已被吊销 | 被其他设备挤下线、主动登出或修改密码后继续请求 |
| `5000` | 服务繁忙 | 未预期的内部错误 |
| `5001` | 数据操作异常 | 数据库故障 |
...
连接存活期间，`writePump` 默认每 30 秒通过 `VERIFY_TOKEN_REMOTE` 回查账号中心。
如果令牌已被吊销，服务端会在同一个写协程中先发送：

```json
{"type":"kicked","data":"登录状态已失效，请重新登录"}
```

随后发送 WebSocket 关闭帧并清理连接。客户端收到 `kicked` 后应停止自动重连、
清理本地登录态并回到登录页；它不能把该事件当成普通网络断线处理。
...
4. **收到 `code=4001` 或 `4007` 时不要自动重试**：可能是被其他端挤下线了，
   应引导用户重新登录，而不是循环重连。
