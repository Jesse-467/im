...
### 7.10 为什么 Chat 也要回查账号中心

一个容易漏掉的点：账号中心开启 `db` 模式**并不会自动让踢下线生效**。
聊天服务的 HTTP 中间件与 WS 网关原本只做本地验签（等价 self 模式），
从不询问账号中心，因此被吊销的令牌在聊天侧仍能继续使用。

所以鉴权拆成了两步，由 `VERIFY_TOKEN_REMOTE` 控制：

```
本地验签（auth.Parse）  →  账号中心 VerifyToken
   零开销，挡伪造/过期        确认未被吊销（可关闭）
```

顺序不可颠倒：本地验签能挡掉绝大多数非法请求，先做它可以避免
无效令牌产生跨服务调用。而回查结果**刻意不缓存**——缓存的窗口期内
被吊销的令牌仍会被放行，与「即时踢下线」的目的冲突。

账号中心不可达时按 fail-closed 拒绝，代价是账号中心故障会导致聊天鉴权
一并失败，换来的是「不会误放行已吊销的令牌」。
...
| `VERIFY_TOKEN_REMOTE` | 鉴权时是否回查账号中心确认令牌未吊销（仅 Chat） | `true` |
